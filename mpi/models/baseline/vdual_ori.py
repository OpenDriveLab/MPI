...
class VDual(nn.Module):
    ...
    def compute_loss(
        ...
        zero_reconstruction = ctx_reconstructions[:, 0, ...]#, ctx_reconstructions[:, 1, ...]

        # Compute mean losses per patch first...
        zero_mse = (zero_reconstruction - targets) ** 2#, (k_reconstruction - k_target) ** 2
        zero_avg_loss_per_patch = zero_mse.mean(dim=-1)#, k_mse.mean(dim=-1)
        # print(mask.sum())
        # Compute mean loss only on *removed* patches and return...
        return zero_avg_loss_per_patch.sum() / torch.ones_like(mask).sum() #, (k_avg_loss_per_patch * mask).sum() / mask.sum()

    def forward(
        self, imgs: torch.Tensor, target_img: torch.Tensor, lang: torch.Tensor, lang_mask: torch.Tensor, mask_ratio: Optional[float] = 0
        ...
        """
        # print(lang.shape, lang_mask.shape)
        visible_ctx_patches, mask, restore_idxs = self.forward_encoder(imgs, lang.squeeze(), lang_mask.squeeze(), mask_ratio)
        ctx_reconstructions = self.forward_decoder(visible_ctx_patches, restore_idxs)
        loss = self.compute_loss(target_img, ctx_reconstructions, mask)
