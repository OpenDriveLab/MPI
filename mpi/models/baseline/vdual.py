...
class VDual(nn.Module):
    ...
    def encode(self, imgs: torch.Tensor, lang: torch.Tensor, lang_mask: torch.Tensor) -> torch.Tensor:
        ...
        lang_embeddings = self.encode_language(lang.squeeze(), lang_mask.squeeze())
    ...
    def forward_encoder(
        ...
        # print(visible_mask.shape, lang_mask.shape)
        multimodal_embedding = torch.cat([visible_patches, lang], dim=1)  # Merge on sequence length...
        multimodal_mask = torch.cat([visible_mask, lang_mask.squeeze()], dim=1)  # Merge on sequence length...
