...
class VCond(nn.Module):
    ...
    def encode(self, img: torch.Tensor, lang: torch.Tensor, lang_mask: torch.Tensor) -> torch.Tensor:
        ...
        # return torch.cat([multimodal_embeddings[:,:-lang_mask.shape[1]].mean(dim=1).unsqueeze(1), \
                        #   multimodal_embeddings[:,-lang_mask.shape[1]:]], dim=1)
    ...
    def forward(
        ...
        visible_patches, mask, restore_idxs = self.forward_encoder(img, lang.squeeze(), lang_mask.squeeze(), mask_ratio)
