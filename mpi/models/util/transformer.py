...
class MotionFormerDecoderLayer_DualFrame(nn.Module):
    def __init__(
            self,
            d_model=192,
            d_ffn=768,
    ...
    def forward(
        ...
        # visual cross-attention
        query2 = self.attn_cross(query = query_inter, key = src[:,:src.shape[1] // 2], value = src[:,:src.shape[1] // 2],
                                 reference_points = reference_points,
                                 spatial_shapes = src_spatial_shapes,
                                 level_start_index = torch.tensor(0).to(src.device))
        query_inter = query_inter + self.dropout_cross(query2)
        query_inter = self.norm_cross(query_inter)

        query3 = self.attn_cross(query = query_inter, key = src[:,src.shape[1] // 2:], value = src[:,src.shape[1] // 2:],
                                 reference_points = reference_points,
                                 spatial_shapes = src_spatial_shapes,
                                 level_start_index = torch.tensor(0).to(src.device))
        query_inter = query_inter + self.dropout_cross(query3)
        query = self.norm_cross(query_inter)
