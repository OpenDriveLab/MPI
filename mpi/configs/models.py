...
class ModelConfig:
    ...
    #eval_checkpoint: str = MISSING

# @MPI Models -- MPI-Small, MPI-Base
@dataclass
class MPIConfig(ModelConfig):
    arch: str = "mpi"
    identifier: str = MISSING
...
@dataclass
class MPISmallConfig(MPIConfig):
    identifier = "mpi-small"
...
# MPI
cs.store(group="model", name="mpi-small", node=MPISmallConfig)
cs.store(group="model", name="mpi-base", node=MPIBaseConfig)
...
cs.store(group="model", name="r3m-vit", node=R3MSmallConfig)
cs.store(group="model", name="r3m-rn50", node=RN3M50Config)
