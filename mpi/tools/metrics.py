...
from mpi.configs import TrackingConfig
...
class Metrics:
    def __init__(
        ...
        resume_id: Optional[str] = None,
        window: int = 32,
        loss_combination : dict = {'add_rec_loss': True,
                            'add_box_loss': True,
                            'add_contra_loss': True},
    ) -> None:
        ...
        if self.model_arch == "mpi":
            if self.loss_combination['add_rec_loss']:
                self.tracker.update(
                    {
                        "reconstruction_loss": deque(maxlen=self.window),
                    })
            if self.loss_combination['add_box_loss']:
                self.tracker.update(
                    {
                        "box_loss": deque(maxlen=self.window),
                    })
            if self.loss_combination['add_contra_loss']:
                self.tracker.update(
                    {
                        "contra_loss": deque(maxlen=self.window),
                    })
                    
        elif self.model_arch == "v-mvp":
    ...
    def push(self, epoch: int) -> str:
        ...
        if self.model_arch == "v-mvp":
            ...
            if self.loss_combination['add_box_loss']:
                metrics.update(
                    {
                        "Pretrain/Box Loss": itemized["box_loss"],
                    })
            if self.loss_combination['add_contra_loss']:
                metrics.update(
                    {
                        "Pretrain/Contrastive Loss": itemized["contra_loss"],
                    })
                    #"Pretrain/Contra Loss": itemized["contra_loss"],
            self.log(
                self.global_step,
                metrics=metrics,
            )
    ...
    def push_epoch(self, epoch: int, val_loss: torch.Tensor) -> Tuple[str, torch.Tensor, int]:
        ...
        epoch_status = (
            f"[Epoch {epoch:03d}] Global Step {self.global_step:06d} =>> LR :: {lr:.6f} -- Loss :: {loss.item():.4f} "
            f"-- Val Loss :: {val_loss.item():.4f} -- Total Time (sec) :: {duration}"
        )
